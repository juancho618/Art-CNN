...
class Model(object):
    ...
    def inference(self, images, keep_prob, train):
        ...
        self.pool1 = self._max_pool(self.conv1_2, 'pool1', debug)#32

        self.conv2_1 = self._conv_layer(self.pool1, "conv2_1")
        self.conv2_2 = self._conv_layer(self.conv2_1, "conv2_2")
        self.pool2 = self._max_pool(self.conv2_2, 'pool2', debug)#16

        self.conv3_1 = self._conv_layer(self.pool2, "conv3_1")
        self.conv3_2 = self._conv_layer(self.conv3_1, "conv3_2")
        self.conv3_3 = self._conv_layer(self.conv3_2, "conv3_3")
        self.pool3 = self._max_pool(self.conv3_3, 'pool3', debug)#8

        self.conv4_1 = self._conv_layer(self.pool3, "conv4_1")
        self.conv4_2 = self._conv_layer(self.conv4_1, "conv4_2")
        self.conv4_3 = self._conv_layer(self.conv4_2, "conv4_3")
        self.pool4 = self._max_pool(self.conv4_3, 'pool4', debug)#4

        self.conv5_1 = self._conv_layer(self.pool4, "conv5_1")
        self.conv5_2 = self._conv_layer(self.conv5_1, "conv5_2")
        self.conv5_3 = self._conv_layer(self.conv5_2, "conv5_3")
        self.pool5 = self._max_pool(self.conv5_3, 'pool5', debug)#2
        ...
        print('fully conv 7', self.fc7.shape)

        self.score_fr = self._fc_layer(self.fc7, "score_fr",
                                           num_classes=num_classes,
                                           relu=False)

        print('core_fr',  self.score_fr.shape)
        #Deconvolution Phase
        self.upscore = self._upscore_layer( self.score_fr, #self.fc7,
                                            shape=tf.shape(images), #original tf.shape(self.pool4)
                                            num_classes=1,#512
                                            debug=debug, name='upscore',
                                            ksize=64, stride=32) # original: ksize=4, stride=2
        
        print('upscore shape',self.upscore.shape)
        reshape = tf.reshape(self.upscore, [-1,64,64]) 
        return reshape
    ...
    def _fc_layer(self, bottom, name, num_classes=None,
        ...
        with tf.variable_scope(name) as scope:
            ...
            if name == 'fc6':
                ...
                filt = self.get_fc_weight_reshape(name, [1, 1, 4096, 255],#1000
                                                  num_classes=num_classes)
            else:
    ...
    def _upscore_layer(self, bottom, shape,
        ...
        with tf.variable_scope(name):
            ...
            if shape is None:
                ...
                new_shape = [shape[0], shape[1], shape[2], num_classes] # when the shape is defined!
            output_shape = tf.stack(new_shape)

            logging.debug("Layer: %s, Fan-in: %d" % (name, in_features))
            f_shape = [ksize, ksize, num_classes, in_features]
            print('fshape', f_shape)
            # create
    ...
    def get_conv_filter(self, name):
        ...
        if not tf.get_variable_scope().reuse:
            weight_decay = tf.multiply(tf.nn.l2_loss(var), self._learning_rate ,
                                       name='weight_loss')
    ...
    def get_deconv_filter(self, f_shape):
        width = f_shape[0]
        height = f_shape[1]
        f = ceil(width/2.0) #originally ceil
        c = (2 * f - 1 - f % 2) / (2.0 * f)
